                ...
                <div class="container-fluid ">
                    ...
                    <a href="{{ route('pelanggan.index') }}" class="btn btn-secondary btn-icon-split" style="margin-bottom: 10px; padding: 0;">
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form action="{{ route('pelanggan.update', $pelanggan->id_pengguna) }}" method="post">
                                    @csrf
                                    @method('PATCH')
                                    <div class="mb-3">
                                        <label for="nama" class="form-label">Nama Kain</label>
                                        <input type="text" class="form-control" id="nama" name="nama" value="{{ old('nama',$pelanggan->nama_pengguna) }}">
                                      </div>
                                      <div class="mb-3">
                                        <label for="user" class="form-label">Username</label>
                                        <input type="text" class="form-control" id="user" name="user" value="{{ old('nama',$pelanggan->username) }}"></input>
                                      </div>
                                      <div class="mb-3">
                                        <label for="pass" class="form-label">Password</label>
                                        <input type="text" class="form-control" id="pass" name="pass" value="{{ old('nama',$pelanggan->password) }}"></input>
                                      </div>
                                      <div class="mb-3">
                                        <label for="email" class="form-label">Email</label>
                                        <input type="number" class="form-control" id="email" name="email" value="{{ old('nama',$pelanggan->email) }}"></input>
                                      </div>
